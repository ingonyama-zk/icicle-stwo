...
impl<T: std::fmt::Debug + Clone + Default> ColumnOps<T> for IcicleBackend {
...
impl DeviceColumn {
    ...
    //     let length = self.len();
    //     unsafe {
    //         let s_data =  transmute(std::slice::from_raw_parts_mut(self.data.as_mut_ptr(), length));
    //         mem::forget(self);
    //         SecureColumn { s_data, length }
    //     }
    // }
}
