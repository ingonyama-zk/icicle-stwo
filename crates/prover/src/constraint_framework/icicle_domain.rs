...
impl<'a> IcicleDomainEvaluator<'a> {
    #[allow(dead_code)]
    pub fn new(
        trace_eval: &'a TreeVec<Vec<&CircleEvaluation<IcicleBackend, BaseField, BitReversedOrder>>>,
        row: usize,
...
impl EvalAtRow for IcicleDomainEvaluator<'_> {
    type F = BaseField;
    type EF = SecureField;
